...
#if WNC_DEL // WNC Steve 2013/05/09, moved to "u-boot-2012.10-rc3/arch/arm/include/asm/iproc/iproc_nand.h"
typedef struct {
    ...
    int cs) 
{
    ...
    /* MICRON MT29F8G08ABACA has 8-bit ECC (strap table value 9) Check for either
	   normal or ONFI ID */
	   if (inand[cs].spare_area_bytes == 28 && inand[cs].page_size == 4096 && inand[cs].device_size == 1024 &&
				   (inand[cs].device_id == 0x2cd390a6 || inand[cs].device_id == 0x4f4e4649))
	   {
		   ...
		   printf("Overriding MT29F8G08ABACA strap options : strap_type=%d\n", strap_type);
               }
           }
           else if (strap_type == 0 ||
    ...
    inand[cs].ecc_strength = ecc_level;
    ...
    uint32_t data_len)
{
	...
	if (ecc_errors > inand[cs].ecc_strength)
...
static int iproc_nand_read_page(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int oob_requred, int page)
{
	int rv;
...
static int iproc_nand_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf, int oob_required)
{
...
static int iproc_nand_write_page(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf, int oob_required, int page, int cached, int raw)
{
	uint64_t page_addr = (uint64_t)page * inand[0].page_size;
	int rv;
